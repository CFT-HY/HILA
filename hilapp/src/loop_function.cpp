...
#define LOOP_FUNC_DEBUG
    ...
    llvm::errs() << "LOOP FUNC " << D->getNameAsString() << " with " << D->getNumParams()
                 << " parameters and " << Call->getNumArgs() << " arguments\n";
...
void TopLevelVisitor::handle_constructor_in_loop(Stmt *s) {
    ...
    if (handle_decl) {
        if (decl->isTrivial()) {
            llvm::errs() << "TRIVIAL CONSTRUCTOR " << decl->getNameAsString()
                         << " DO NOTHING\n";
        } else {
...
bool TopLevelVisitor::handle_special_loop_function(CallExpr *Call) {
    ...
    if (CXXMemberCallExpr *MCall = dyn_cast<CXXMemberCallExpr>(Call)) {
        ...
        if (objtype == "const class X_index_type" ||
            objtype == "const class lattice_struct *") {
...
bool TopLevelVisitor::handle_loop_function_if_needed(FunctionDecl *fd) {
    ...
    if (handle_decl) {
        if (fd->isTrivial()) {
            llvm::errs() << "TRIVIAL LOOP FUNCTION " << fd->getNameAsString()
                         << " NO HANDLING\n";
        } else {
            ...
            llvm::errs() << "decl: "
                         << getRangeText(srcMgr, fd->getSourceRange().getBegin(),
...
bool TopLevelVisitor::loop_function_check(Decl *d) {
    ...
    if (fd) {
        ...
        // Argument of hasBody becomes the pointer to definition if it is in this
        // compilation unit needs to be const FunctionDecl *
        ...
        if (fd->hasBody(cfd)) {
            ...
            // Now function has no body - could be in other compilation unit or in system
            // library.
