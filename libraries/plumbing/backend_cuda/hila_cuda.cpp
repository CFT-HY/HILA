...
curandState *gpurandstate;
__device__ curandState *d_gpurandstate;
...
void hila::seed_device_rng(unsigned long seed) {
    ...
    cudaMalloc(&gpurandstate, n_sites * sizeof(curandState));
...
#ifdef __CUDA_ARCH__
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    return curand_uniform(&d_gpurandstate[x]);
#else
    return mersenne();
#endif
}
...
void backend_lattice_struct::setup(lattice_struct *lattice) {
    ...
    check_device_error("cudaMemcpy device coordinate array");
    free(tmp);
    ...
    check_device_error("cudaMemcpy to size or volume");
}
