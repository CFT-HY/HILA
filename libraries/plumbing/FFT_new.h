...
inline void increment_current_coord( CoordinateVector & current, direction odirs[NDIM-1] ) {
...
inline void FFT_field_complex(const Field<T> & input, Field<T> & result, 
    ...
    cmplx_t * const RESTRICT collect_buffer = (cmplx_t *)memalloc(local_volume*elements*sizeof(cmplx_t));
    cmplx_t * const RESTRICT receive_buffer = (cmplx_t *)memalloc(local_volume*elements*sizeof(cmplx_t));
    ...
    foralldir(dir) {
        ...
        init_fft_direction(dir, elements, sizeof(T), fftdir,
                           collect_buffer, receive_buffer );
...
template<typename T>
void FFT_field(const Field<T> & input, Field<T> & result, fft_direction fdir = fft_direction::forward){
