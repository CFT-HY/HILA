...
SUBNODE_LAYOUT needs to be defined to use this
  ...
  public:
    ...
    vectorized_lattice_struct(lattice_struct *_lattice) {
        ...
        output0 << "Setting up lattice struct with vector of size " << vector_size
                << " elements\n";
    ...
    void get_neighbours_and_local_halo() {
        ...
        for (Direction d = (Direction)0; d < NDIRS; ++d) {
            ...
            for (int i = 0; i < v_sites; i++) {
                int j = vector_size *
                        i; // the "original lattice" index for the 1st site of vector
            ...
            if (n_halo_vectors[d] > 0 && is_boundary_permutation[abs(d)]) {
                halo_index[d] =
                    (unsigned *)memalloc(n_halo_vectors[d] * sizeof(unsigned));
                int j = 0;
                for (int i = 0; i < v_sites; i++) {
                    if (neighbours[d][i] >= v_sites) {
                        ...
                        for (k = 0; k < vector_size; k++) {
                            if (lattice->neighb[d][i * vector_size + k] <
                                lattice->mynode.sites) {
                                if (!found) {
                                    n = lattice->neighb[d][i * vector_size + k] /
                                        vector_size;
                                    found = true;
                                } else
                                    assert(n == lattice->neighb[d][i * vector_size + k] /
                                                    vector_size);
                            }
                        }
                        assert(n >= 0);
                ...
                for (int i = 0; i < v_sites; i++) {
                    if (neighbours[d][i] >= v_sites) {
                        halo_index[d][j++] =
                            lattice->neighb[d][i * vector_size] / vector_size;
    ...
    void set_coordinates() {
        ...
        for (int i = 0; i < vector_size; i++) {
            ...
            foralldir(d) coordinate_offset[d].insert(i, diff[d]);
        }

        // and then set the coordinate_base with the original coords
        coordinate_base =
            (CoordinateVector *)memalloc(v_sites * sizeof(CoordinateVector));
        for (int i = 0; i < v_sites; i++) {
...
/// at that dir is out of the local volume
    ...
    unsigned field_alloc_size() const { return alloc_size; }
...
struct backend_lattice_struct {
    ...
    void setup(lattice_struct *_lattice) { latticep = _lattice; }
