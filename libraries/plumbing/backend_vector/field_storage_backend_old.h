...
template <> struct vectorize_struct<CoordinateVector, 4> {
    using type = std::array<Vec4i, NDIM>;
};

/// Match coordinate vectors explicitly
template <> struct vectorize_struct<CoordinateVector, 8> {
    using type = std::array<Vec8i, NDIM>;
};

/// Match coordinate vectors explicitly
template <> struct vectorize_struct<CoordinateVector, 16> {
...
template <typename T>
template <typename A>
inline void field_storage<T>::set(const A &value, const int i,
    ...
    if (vlat->split[dir] > 1) {
        ...
        for (Parity p : loop_parities(par)) {
            ...
            for (int idx = 0; idx < hs.nb_index.size(); idx++) {
                ...
                basetype *RESTRICT d =
                    fp + elements * vector_size * (vlat->sites + hs.first_index + idx);
                for (int e = 0; e < elements; e++) {
