...
///            b) characters enclosed by quote marks "..".  These have to pair within the same line.
...
///      
///     std::vector<double> vd = in.get("vector-key1")        or  in.get_double_vector("vector-key1");
///     std::vector<std::string> vs = in.get("vector-key2")   or  in.get_string_vector("vector-key2");
///
...
///          
///          Form (*) also matches a double precision value.  If this matches, return value
///          is -1 and val is assigned to the found value.  For example, code
///               double c_sw;
///               i = in.get_item("c_sw", {"tree","perturbative"},c_sw );
///          on lines 
///               c_sw  perturbative              - returns 1
///               c_sw  1.343                     - returns -1 and variable c_sw is assigned 1.343             
///               c_sw  abcd                      - gives error and quits
///
///      in.close();   - closes the file.  Now "in.open("file")" can be used again.
///          File is also closed when variable "in" goes out of scope.
///
///      NOTE: an empty key "" always matches, and can be used to read values without keys.
///            E.g.  
///                 i = get_int("");  or   i = get("");   reads in next integer.
///            For functions get_int(), get_double(), get_string() the empty string can be left out.
///                
///      NOTE: returned values are always broadcast to all MPI nodes.  No need for the user to do it.
///
////////////////////////////////////////////////////////////////////////
...
class input {
    public:
        ...
        class returntype {
            ...
            operator std::vector<int>() {
                return parent->get_int_vector(label);
            }

            operator std::vector<double>() {
                return parent->get_double_vector(label);
            }

            operator std::vector<std::string>() {
                return parent->get_string_vector(label);
            }
        ...
        int get_item(const std::string &label, const std::vector<std::string> &items, 
                     double *dval = nullptr);
        int get_item(const std::string &label, const std::vector<std::string> &items, double &val);
    ...
    private:
        ...
        template <typename T>
        void get_type_vector(const std::string & label, std::vector<T> & res, 
                             const char * name);
