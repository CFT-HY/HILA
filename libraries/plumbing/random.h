...
enum class device_rng : bool { on = true, off = false };
...
/**
 *@brief Initialize host (CPU) random number generator separately, done implicitly by
 *`seed_random()`
 */
 ...
 * This function can be called from outside or inside site loops (on GPU if the device rng is
 * initialized).
 *
 * Uses
