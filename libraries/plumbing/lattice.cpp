...
bool lattice_struct::is_on_mynode(const CoordinateVector & loc)
{
  ...
  for (int dir=0; dir<NDIM; dir++) {
    ...
    if (d < 0 || d >= mynode.size[dir] ) return false;
  }
  return true;
}
...
unsigned lattice_struct::site_index(const CoordinateVector & loc)
{
  ...
  else return( i/2 + mynode.evensites );  /* odd site */
#else
  return( i );
#endif
}
...
unsigned lattice_struct::site_index(const CoordinateVector & loc, const unsigned nodeid)
{
  ...
  for (dir=NDIM-1; dir>=0; dir--) {
    ...
    if (dir != mynode.subnodes.merged_subnodes_dir) { 
      subl = subl * mynode.subnodes.divisions[dir] 
           + l / subsize[dir];
    }
...
void lattice_struct::create_std_gathers()
{
  ...
  for (direction d=e_x; d<NDIRS; ++d) {
      ...
      for (size_t i=0; i<mynode.sites; i++) {
        if (neighb[d][i] == mynode.sites) {
          ...
          if (l.parity() == EVEN) {
            ...
            if (c_offset+c_even >= (1ULL << 32)) too_large_node = 1;
    ...
    if (c_offset >= (1ULL << 32)) too_large_node = 1;
...
void lattice_struct::initialize_wait_arrays()
{
  ...
  for (size_t i=0; i<mynode.sites; i++) {
    ...
    foralldir(dir) {
      direction odir = -dir;
      if ( neighb[dir][i]  >= mynode.sites ) wait_arr_[i] = wait_arr_[i] | (1<<dir) ;
      if ( neighb[odir][i] >= mynode.sites ) wait_arr_[i] = wait_arr_[i] | (1<<odir) ;
    }
  }
}
...
void lattice_struct::init_special_boundaries() {
  for (direction d=(direction)0; d<NDIRS; ++d) {
    ...
    if (is_up_dir(d) && mynode.min[d] + mynode.size[d] == size(d)) coord = size(d)-1;
    if (is_up_dir(od) && mynode.min[od] == 0) coord = 0;
    ...
    if (coord >= 0) {
      ...
      if (nodes.n_divisions[abs(d)] == 1) {
        ...
        for (unsigned i=0; i<mynode.sites; i++) if (coordinate(i,abs(d)) == coord) {
          // set buffer indices
    ...
    // hila::output << "Node " << hila::myrank() << " dir " << d << " min " << mynode.min << " is_on_edge "
