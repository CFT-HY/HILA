...
class input {
  public:
    ...
    // add an essential variable - triggers complaint if not in parameter file or
    // commandline
    void add_essential(const std::string &);
    // same as above, except defines a default value to be used in case it is not found
    template <typename T> void add_essential(std::string const &, T const &default_value);
    ...
    class returntype {
      public:
        ...
        input *parent;
        operator double() { return parent->values[name]; }
        operator float() { return (float)parent->values[name]; }
        operator int() { return (int)parent->values[name]; }
        operator std::string() { return parent->names[name]; }
    };
