...
void map_node_layout() {
    ...
    if (j >= 0) {
        ...
        fprintf0(outf,
                 "No mod 3 dimensions found, attempt map_node_layout: BLOCK 2^dim\n");
    ...
    if (mynode == 0) {
        if (j >= 0) {
            fprintf(outf,
                    " map_node_layout: logical node blocks 3x2^%d, %d nodes/block\n", na,
                    blocksize);
        } else {
        ...
        for (d = 0; d < NDIM - 1; d++)
            fprintf(outf, "%d x ", bsize[d]);
        fprintf(outf, "%d\n", bsize[NDIM - 1]);
        fprintf(outf, "   Taking directions in order: ");
        for (d = 0; d < NDIM - 1; d++)
            fprintf(outf, "%d ", order[d]);
        fprintf(outf, "%d\n", order[NDIM - 1]);
    }
