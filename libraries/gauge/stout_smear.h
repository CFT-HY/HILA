...
template <typename T, typename atype = hila::arithmetic_type<T>>
void stout_smear1(const GaugeField<T> &U, out_only GaugeField<T> &stout, out_only VectorField<int> &niter,
                  atype coeff) {
    staplesums(U, stout);
    foralldir(d) {
        onsites(ALL) {
            stout[d][X] =
                chexp((U[d][X] * stout[d][X]).project_to_algebra_scaled(-coeff).expand(), niter[d][X]) * U[d][X];
        }
    }
}
...
void stout_smear(const GaugeField<T> &U, out_only std::vector<GaugeField<T>> &stoutlist,
    ...
    stoutlist[0] = U; // original field
    for (int i = 1; i < stoutlist.size(); ++i) {
        stout_smear1(stoutlist[i - 1], stoutlist[i], niterlist[i - 1], coeff);
    }
}
