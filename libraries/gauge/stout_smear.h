...
template <typename T, typename atype = hila::arithmetic_type<T>>
void stout_smear1(const GaugeField<T> &U, out_only GaugeField<T> &stout,
                  out_only VectorField<T> &stap, atype coeff) {
    staplesums(U, stap);
    foralldir(d) {
        onsites(ALL) {
            stout[d][X] = chexp((U[d][X] * stap[d][X]).project_to_algebra_scaled(-coeff).expand()) * U[d][X];
        }
    }
}
...
void stout_smear(const GaugeField<T> &U, out_only std::vector<GaugeField<T>> &stoutlist,
                 out_only std::vector<VectorField<T>> &staplist, atype coeff) {
    ...
    for (int i = 1; i < stoutlist.size(); ++i) {
        stout_smear1(stoutlist[i - 1], stoutlist[i], staplist[i - 1], coeff);
    }
}
...
void stout_smear1k(const GaugeField<T> &U, out_only GaugeField<T> &stout,
                   out_only VectorField<T> &stap, out_only VectorField<T> &stoutk, atype coeff) {
    staplesums(U, stap);
    foralldir(d) {
        onsites(ALL) {
            stout[d][X] = chexpk((U[d][X] * stap[d][X]).project_to_algebra_scaled(-coeff).expand(),
                                 stoutk[d][X]) * U[d][X];
        }
    }
}
...
void stout_smeark(const GaugeField<T> &U, out_only std::vector<GaugeField<T>> &stoutlist,
                  out_only std::vector<VectorField<T>> &staplist,
                  out_only std::vector<VectorField<T>> &stoutklist, atype coeff) {
    ...
    stoutlist[0] = U; // original field
    for (int i = 1; i < stoutlist.size(); ++i) {
        stout_smear1k(stoutlist[i - 1], stoutlist[i], staplist[i - 1], stoutklist[i - 1], coeff);
    }
}

template <typename T, typename atype = hila::arithmetic_type<T>>
void stout_smeark_force(
    const std::vector<GaugeField<T>> &stoutlist, const std::vector<VectorField<T>> &staplist,
    const std::vector<GaugeField<T>> &stoutklist, const VectorField<Algebra<T>> &K,
    ...
    for (int i = stoutlist.size() - 2; i >= 0; --i) {
        ...
        foralldir(d1) foralldir(d2) if (d1 != d2) {
            onsites(ALL) {
                /*
                T U2, U4, U2U3, U2U3U4, U2L2U4;

                U2 = U[d2][X + d1];
                U4 = U[d2][X].dagger();

                U2U3 = U2 * U[d1][X + d2].dagger();
                U2U3U4 = U2U3 * U4;
                U2L2U4 = U2 * K1[d1][X + d2].dagger() * U4;

                K21[X] = U4 * K1[d1][X] * U2U3;
                K23[X] = U[d1][X] * U2L2U4;

                K22[X] = U2L2U4 * U[d1][X] + U2U3U4 * K1[d1][X];
                K24[X] = K23[X] + K1[d1][X] * U2U3U4;
                */
                T U2, U4, tM1;
...
void stout_smeark_force_b(const std::vector<GaugeField<T>> &stoutlist,
