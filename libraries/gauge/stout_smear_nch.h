#ifndef STOUT_SMEAR_NCH_H_
#define STOUT_SMEAR_NCH_H_
...
/////////////////////////////////////////////////////////////////////////////
/// Do stout smearing for the gauge fields using non-Cayley-Hamilton exponentiation
 ...
 */
template <typename T>
void nch_staplesums(const GaugeField<T> &U, out_only GaugeField<T> &staples) {
    Field<T> tF1, lw1;
    foralldir(d1) {
...
void nch_stout_smear1(const GaugeField<T> &U, out_only GaugeField<T> &stout, atype coeff) {
    nch_staplesums(U, stout);
    foralldir(d) {
        onsites(ALL) {
            stout[d][X] =
...
template <typename T, typename atype = hila::arithmetic_type<T>>
void nch_stout_smear1(const GaugeField<T> &U, out_only GaugeField<T> &stout,
                      out_only VectorField<T> &stap, atype coeff) {
    nch_staplesums(U, stap);
    foralldir(d) {
        onsites(ALL) {
            stout[d][X] =
                exp((U[d][X] * stap[d][X]).project_to_algebra_scaled(-coeff).expand()) * U[d][X];
            stout[d][X].reunitarize();
        }
    }
}
...
template <typename T, typename atype = hila::arithmetic_type<T>>
void nch_stout_smear(const GaugeField<T> &U, GaugeField<T> &stout, atype coeff, int iter) {
    GaugeField<T> tmp;
    if (iter % 2 == 0) {
        stout = U;
        for (int i = 0; i < iter / 2; ++i) {
            nch_stout_smear1(stout, tmp, coeff);
            nch_stout_smear1(tmp, stout, coeff);
        }
    } else {
        tmp = U;
        for (int i = 0; i < iter / 2; ++i) {
...
template <typename T, typename atype = hila::arithmetic_type<T>>
void nch_stout_smear(const GaugeField<T> &U, out_only std::vector<GaugeField<T>> &stoutlist,
    ...
    for (int i = 1; i < stoutlist.size(); ++i) {
        nch_stout_smear1(stoutlist[i - 1], stoutlist[i], staplist[i - 1], coeff);
    }
}

template <typename T, typename atype = hila::arithmetic_type<T>>
void nch_stout_smear_force(const std::vector<GaugeField<T>> &stoutlist,
        ...
        foralldir(d1) {
            ...
            onsites(ALL) {
                ...
                mult_exp(tplaqs.project_to_algebra_scaled(-coeff).expand(),
                         U[d1][X] * U0[d1][X].dagger() * KS[d1][X].expand(), mtexp, mdtexp);
