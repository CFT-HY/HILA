  ...
  for (d=0; d<NDIRS; d++) {
    ...
    for(int i=0; i<this_node.sites; i++) {
      ...
      if (is_up_dir(d)) {
        ...
        direction k = opp_dir(d);
        l[k] = (l[k] + size[k] - 1) % size[k];
      }
      if (is_on_node(l)) {
        neighbour[d][i] = site_index(l);
      } else {
        // off-node
        unsigned node_id = node_number(l);
        int n;
        bool found=false;
        for (n=0; !found && n<comminfo[d].from_nodes.size(); n++)
          if (comminfo[d].from_nodes[n].id == node_id) found = true;
        if (!found) {
          // no new node, add
          comm_node cn;
          cn.id = node_id;
          cn.n = cn.n_even = cn.n_odd = 0;
          n = comminfo[d].from_nodes.size();
          comminfo[d].from_nodes.push_back(cn);
        }
        // now add info
        comminfo[d].from_nodes[n].n++;
        if (location_parity(l) == parity::even)
          comminfo[d].from_nodes[n].n_even++;
        else 
          comminfo[d].from_nodes[n].n_odd++;
        comminfo[d].num++;
      }
    }
    
    for (comm_node & cn : comminfo[d].from_nodes) {
      // get the offsets to the buffers
      cn.buffer = c_offset;
      c_offset += cn.n;
      
      // and finish setting up neighbour arrays

      for (parity p=EVEN; p<=ODD; p++) {
        for (int n=0; n<comminfo[d].num; n++) 
          if (cn.id == )
      }
