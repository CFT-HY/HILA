...
// special staplesum routine which moves upper d,t -staple from t=1 to t=0
// calculates the staplesums only for t==0 timeslice!

template <typename T>
void staplesum_T_slice(const GaugeField<T> &U, Field<T> &staples, Parity par, Direction d1) {
    ...
    foralldir(d2) if (d2 != d1) {
        ...
        onsites(opp_parity(par)) {
            if (X.t() == lattice.size(e_t) - 1 || X.t() == 0) {
        ...
        if (d2 == e_t) {
            onsites(ALL) {
                if (X.t() == 1) {
                    upper[X] = U[d2][X] * U[d1][X + d2] * U[d2][X + d1].dagger();
                }
            }
            // shift upper from t==1 plane
            onsites(par) if (X.t() == 0) {
                staples[X] += upper[X + e_t] + lower[X - e_t];
            }
...
double measure_plaq_db(const GaugeField<group> &U, bool oddT = false, double db = 0.0) {
        ...
        if (db == 0.0) {
            ...
            onsites(ALL) {
                if (!oddT || X.t() != 0) {
                    ...
                    plaq +=
                        c * (1.0 - real(trace(U[dir1][X] * U[dir2][X + dir1] *
                                              U[dir1][X + dir2].dagger() * U[dir2][X].dagger())) /
                                       group::size());
                }
            }
...
void measure_stuff(const GaugeField<group> &U, const parameters &p) {
    ...
    double pnorm = lattice.volume() * NDIM * (NDIM - 1) / 2;
    if (p.odd_T_extent)
        pnorm *= ((double)lattice.size(e_t) - 1) / lattice.size(e_t);
...
// enclose generic update to this function

template <typename group>
inline void suN_update(group &U, const group &staples, const double beta, const bool relax) {

    if (relax) {

#ifdef SUN_OVERRELAX_dFJ
        suN_overrelax_dFJ(U, staples, beta);
#else
        suN_overrelax(U, staples);
#endif

    } else {

        suN_heatbath(U, staples, beta);
    }
}
...
void update_parity_dir(GaugeField<group> &U, const parameters &p, Parity par, Direction d,
    ...
    if (!p.odd_T_extent) {
        ...
        onsites(par) suN_update(U[d][X], staples[X], p.beta, relax);

    } else {
        // now this branch goes to fake T extension
        // first other besides t=0 or 1 slices, more or less std update

        onsites(par) {
            if (X.t() != 0 && (d == e_t || X.t() != 1)) {
                suN_update(U[d][X], staples[X], p.beta, relax);
            }
        }

        // then t=0,1 slices if d!=e_t
        if (d != e_t) {
            // need to get staples again - shifted suitably!
            // staples only for t==0 slice
            staplesum_T_slice(U, staples, par, d);

            onsites(par) if (X.t() == 0) {
                suN_update(U[d][X], staples[X], p.beta, relax);
            }

            // Copy now U from t=0 to t=1
#pragma hila safe_access(U[d])
            onsites(opp_parity(par)) if (X.t() == 1) {
                U[d][X] = U[d][X - e_t];
            }
            // set the t-links to unity, should be that in any case?
            onsites(ALL) if (X.t() == 0) {
                U[e_t][X] = 1;
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    if (lsize[e_t] % 2) {
        hila::out0 << "USING FAKE ODD - N_t LATTICE, logical " << lsize[e_t] << " physical "
                   << ++lsize[e_t] << '\n';
        p.odd_T_extent = true;
    } else {
        p.odd_T_extent = false;
    }
