...
void spectraldensity_surface(std::vector<float> &surf,
                             std::vector<double> &npow,
                             std::vector<int> &hits) {
  ...
  if (first) {
    ...
    fftwplan = fftw_plan_dft_2d(lattice.size(e_y), lattice.size(e_x),
...
template <typename T>
void measure_polyakov_field(const Field<T> &Ut,
                            Field<Complex<float>> &polyakov_field) {
  Field<T> polyakov = Ut;
...
////////////////////////////////////////////////////////////////////
template <typename T>
void smear_polyakov_field(Field<T> &polyakov_field, int nsmear,
                          float smear_coeff, double twist) {
  if (nsmear > 0) {
    Field<T> pl2 = 0;
    ...
    for (int i = 0; i < nsmear; i++) {
      onsites(ALL) if (X.coordinate(e_t) == 0) {
        ...
        if (X.coordinate(e_z) == 0)
          pl2[X] += smear_coeff * (expi(-2 * M_PI * twist / NCOLOR) *
 ...
 */
template <typename T>
void measure_polyakov_profile(Field<T> &polyakov_field,
                              std::vector<T> &surface_origin_profile,
                              std::vector<T> &surface_average_profile) {
  ReductionVector<T> p_surface_average(lattice.size(e_z)),
      p_origin(lattice.size(e_z));
  ...
  std::transform(
      surface_average_profile.begin(), surface_average_profile.end(),
...
void measure_polyakov_surface(GaugeField<group> &U, const parameters &p,
                              int traj) {
  ...
  if (first) {
    ...
    for (int sl = 0; sl < p.n_smear.size(); sl++) {
      print_formatted_numbers(
          dummy, "PRO " + std::to_string(sl) + " Origin polyakov smeared",
          true);
      print_formatted_numbers(
          dummy, "PRO " + std::to_string(sl) + " Average polyakov smeared",
          true);
      first = false;
    }
  }
  ...
  for (int sl = 0; sl < p.n_smear.size(); sl++) {
      ...
      for (int j = 0; j < p.z_smear.at(sl); j++) {
        ...
        onsites(ALL) if (X.coordinate(e_t) == 0) {
          ...
          if (X.z() == 0)
            ...
            sub_polyakov_field[X] = polyakov_field_z[X] +
    ...
    measure_polyakov_profile(polyakov_field_z, surface_origin_profile,
                             surface_average_profile);
    ...
    for (int y = 0; y < lattice.size(e_y); y++) {
      ...
      if (hila::myrank() == 0) {
        for (int x = 0; x < lattice.size(e_x); x++) {
          ...
          } else if (interpolated_min <
                     minloc_global - 0.5 * lattice.size(e_z)) {
            interpolated_min += lattice.size(e_z);
          }
