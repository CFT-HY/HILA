...
void measure_polyakov_surface(GaugeField<group> &U, const parameters &p, int traj) {
    ...
    if (first) {
        ...
        for (int sl = 0; sl < p.n_smear.size(); sl++) {
            ...
            print_formatted_numbers(
                dummy, "PRO " + std::to_string(smear) + " Origin polyakov smeared", true);
            print_formatted_numbers(
                dummy, "PRO " + std::to_string(smear) + " Average polyakov smeared", true);
            first = false;
        }
    }
    ...
    for (int sl = 0; sl < p.n_smear.size(); sl++) {
        ...
        print_formatted_numbers(surface_origin_profile,
                                "PRO " + std::to_string(smear) + " Origin polyakov smeared", false);
        print_formatted_numbers(surface_average_profile,
                                "PRO " + std::to_string(smear) + " Average polyakov smeared",
                                false);
        float min = 1e8;
        int minloc_global;
        for (int i = 0; i < surface_average_profile.size(); i++) {
