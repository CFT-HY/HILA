...
template <typename group>
void update_multicanonical(GaugeField<group> &U, const parameters &p, bool relax) {
  foralldir(d) {
    for (Parity par : {EVEN, ODD}) {
      if (d==e_t) {
        if (p.muca_poly) {
          ...
          //hila::out0 << "OP old: " << OP_old.back() << " OP new: " << OP.back() << std::endl;
...
int main(int argc, char **argv) {
  ...
  double initial_state = par.get("initial_state");
