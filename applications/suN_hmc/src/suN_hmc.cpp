...
void measure_topo_charge_and_energy_log(const GaugeField<group> &U, atype &qtopo_out,
    ...
    foralldir(dir1) foralldir(dir2) if (dir1 < dir2) {
        ...
        onsites(ALL) {
            ...
            F[k][X] = (tF0[X] + tF1[X - dir1] +
                       U[dir2][X - dir2].dagger() * (tF0[X - dir2] + tF1[X - dir1 - dir2]) *
                           U[dir2][X - dir2]) *
                      0.25;
        }
        ++k;
...
void measure_topo_charge_and_energy_clover(const GaugeField<group> &U, atype &qtopo_out,
    ...
    foralldir(dir1) foralldir(dir2) if (dir1 < dir2) {
        onsites(ALL) {
            ...
            F[k][X] =
                0.25 * (U[dir1][X] * U[dir2][X + dir1] * (U[dir2][X] * U[dir1][X + dir2]).dagger() -
                        (U[dir1][X - dir1 - dir2] * U[dir2][X - dir2]).dagger() *
                            U[dir2][X - dir1 - dir2] * U[dir1][X - dir1] +
                        U[dir2][X] * (U[dir2][X - dir1] * U[dir1][X - dir1 + dir2]).dagger() *
                            U[dir1][X - dir1] -
                        U[dir1][X] * (U[dir1][X - dir2] * U[dir2][X + dir1 - dir2]).dagger() *
                            U[dir2][X - dir2]);
...
void get_force_bp(const GaugeField<group> &U, VectorField<Algebra<group>> &K) {
    ...
    foralldir(d1) {
        foralldir(d2) if (d2 > d1) {
            onsites(ALL) {
                fmatp[X] =
                    get_bp_UAmat(U[d1][X] * U[d2][X + d1] * (U[d2][X] * U[d1][X + d2]).dagger());
...
void update_E_bp(const GaugeField<group> &U, VectorField<Algebra<group>> &E, const parameters &p,
                 atype delta) {
    // compute force for BP action for n=2 according to Eq. (B5) of arXiv:2306.14319 and use it to
    // evolve E
    Field<group> fmatp;
    Field<group> fmatmd1;
    Field<group> fmatmd2;
...
template <typename group, typename atype = hila::arithmetic_type<group>>
atype measure_action_bp(const GaugeField<group> &U, const VectorField<Algebra<group>> &E,
                        const parameters &p) {
...
void measure_stuff(const GaugeField<group> &U, const VectorField<Algebra<group>> &E) {
    ...
    hila::out0 << string_format("MEAS % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e", plaqbp, plaq, e2,
                                poly.real(), poly.imag())
               << '\n';
}
...
void measure_wflow_stuff(const GaugeField<group> &V, atype flow_l, atype t_step) {
    ...
    if (first) {
        ...
        hila::out0 << "Legend WFLMEAS  f.-scale  BP-S-plaq  S-plaq  t^2*E_plaq  t^2*E_log  "
                      "Qtopo_log  t^2*E_clov  Qtopo_clov  [t step size]\n";
        first = false;
    }
    auto plaqbp = measure_plaq_bp(V) / (lattice.volume() * NDIM * (NDIM - 1) /
                                        2); // average bulk-prevention plaquette action
    ...
    hila::out0 << string_format(
                      "WFLMEAS % 7.3f % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e % 0.6e   [%0.5f]",
                      flow_l, plaqbp, plaq, pow(flow_l, 4) / 64.0 * eplaq,
                      pow(flow_l, 4) / 64.0 * elog, qtopolog, pow(flow_l, 4) / 64.0 * ecl, qtopocl,
                      t_step)
               << '\n';
}
...
template <typename group, typename atype = hila::arithmetic_type<group>>
atype do_wilson_flow_adapt(GaugeField<group> &V, atype l_start, atype l_end, atype tstep = 0.001,
                           atype atol = 1.0e-7, atype rtol = 1.0e-7) {
            ...
            if (p.wflow_freq > 0 && trajectory % p.wflow_freq == 0) {
                ...
                if (p.wflow_l_step > 0) {
                    ...
                    for (int i = 0; i < nflow_steps; ++i) {
                        ...
                        t_step =
                            do_wilson_flow_adapt(V, i * p.wflow_l_step, (i + 1) * p.wflow_l_step,
                                                 t_step, p.wflow_a_accu, p.wflow_r_accu);

                        measure_wflow_stuff(V, (i + 1) * p.wflow_l_step, t_step);
                    }

                    hila::out0 << "Wflow_end " << wtrajectory << "    time " << std::setprecision(3)
                               << hila::gettime() - wtime << '\n';
                }
            }
        }
