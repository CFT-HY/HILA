...
void get_clover_leaves(const GaugeField<group>& U,Direction d1,Direction d2,Field<group>(&C)[4],Field<group>& Csum) {
    // assignes the clover leave matrices in counter-clockwise order to C[0],C[1],C[2],C[3] and sets Csum 
    // to the full Clover matrix, i.e. to the anti-hermitian tracless part of (C[0]+C[1]+C[2]+C[3])/4 
...
atype measure_s_clover_action(const GaugeField<group>& U) {
    ...
    foralldir(dir1) foralldir(dir2) if(dir1<dir2) {
        ...
        onsites(ALL) {
            stot+=-0.5*real(trace(Csum[X]*Csum[X]));
        }
    }
...
void get_force_clover_action_add(const GaugeField<group>& U,VectorField<Algebra<group>>& K, atype eps=1.0) {
    ...
    foralldir(dir1) foralldir(dir2) if(dir1<dir2) {
        ...
        // define the for each clover leave matrix the correspondin path of gauge links
        ...
        //           = 0.5*\sum_k (Tr((d_{Y,dir}C[k][X])*Csum[X]) - Tr((d_{Y,dir}C[k][X].dagger())*Csum[X]))
        //           = \sum_k Tr((d_{Y,dir}C[k][X])*Csum[X])
        for(int k=0; k<4; ++k) {
