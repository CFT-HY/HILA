...
bool in_specialization_db( const std::string & decl_in, std::string & here ) {
  ...
  if (s == nullptr) {
    ...
    struct stat status;
    if (stat(s->file.c_str(), &status) == 0 &&  status.st_mtim.tv_sec > s->timestamp) {
      // If the mod time of the file is later than timestamp, future compilation expected:
      // steal the specialization
...
void write_specialization_db() {
  ...
  // have to load db in any case, because need to strip possible specializatons from this file

  if (!db_loaded) load_spec_db();

  std::ofstream db(specialization_db_filename, std::ios_base::out | std::ios_base::trunc );
  if (db.is_open()) {
